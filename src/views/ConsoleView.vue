...
import axios from '@/api/axios'; // Asumiendo que tu axios tiene la configuración base
...
const handleMakeOp = async () => {
  ...
  try {
    // Usamos el endpoint que configuramos en el backend anteriormente
    const response = await axios.post(`/api/minecraft/op/${opPlayer.value}`);
    alert(`Comando ejecutado: ${response.data.response}`);
    opPlayer.value = '';
    // Podrías opcionalmente refrescar la consola aquí si tienes un bus de eventos
  } catch (error) {
...
const quickCommand = async (cmd) => {
  ...
  try {
    await mcStore.executeCommand(cmd); // Usando la función de tu store
  } catch (error) {
    console.error(error);
  }
};
